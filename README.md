...
- **输出带理由**：要求模型输出 `ALLOW`、`DENY - <理由>` 或 `ASK - <理由>`。理由会写进
  `decisions.jsonl` 的 `model_reason` 字段并进 `--stats` 的原因分布 —— 这样「模型为什么
  拦」有据可查（早期只存一个词，无从追查，遇到误判只能靠猜）。
- **三档语义（`never_ask: false`，默认）**：
  - `ALLOW` 放行；
  - `DENY` 明确有害 → **硬阻断**，理由回给 agent，用户确认后重试可放行；
  - `ASK` 模型确实判断不了（例如授权发生在更早的对话里）→ **交人工弹窗**。
  提示词里明确写了「ASK 会打断用户、不是保险手段」，避免模型拿它当默认。
  设 `never_ask: true` 会取消 ASK，模型必须二选一 —— **不建议**，见下。
- **为什么默认保留 ASK**：实测按项目统计，用户自己的项目里只有 **2%~4%** 的命令会弹窗，
  而本会话里模型给出的 ASK 理由**逐条都是误判**（「未授权推送 GitHub」「读取应用内部代码」
  「修改工作区外的配置文件」——都是用户刚刚要求过的事）。当 ASK 时只是多点一下；改成 DENY
  就会**直接卡住用户交代的任务**，代价远大于一次点击。真正该优化的是「落模型的命令占
  72%~82%」这件事（延迟与卡片闪烁），而不是把询问换成阻断。
- **授权上下文窗口**：默认取最近 4 条用户发言（1600 字符）作为 TRUSTED —— 窗口太小是
  模型误判的主因（它看不到几轮前的授权）。用 `context_messages` / `context_chars` 调。
- 写脚本不执行是无害的：`cat > /tmp/x.py <<'EOF' … EOF` 这类**只写文件**的命令应该放行，
  只有脚本内容确实会删工作区外数据或外发数据时才拦。
...
### 本地层理解的 shell 语法（这些不落模型）

只读白名单是「按片段形状」匹配的，所以壳语法必须先被正确拆解，否则 `if`、`> file`
这类骨架会把整条命令推给模型（白等 2~4 秒）。目前本地层理解：

| 构造 | 处理 |
| --- | --- |
| 控制流 | `if/then/elif/else/fi`、`while/until/do/done`、`!`、`{ }`、`time` 作为骨架剥掉；`for`/`case`/`select` 的**头部**整段是骨架。**条件本身不剥**（`if <cmd>` 里的命令照常审） |
| 重定向 | 目标是 `/dev/null`、`/tmp` 族、或**相对路径**时视为透明（写文件本身不执行任何东西）。**命令里有 `cd` 时，所有 `cd` 目标必须解析后仍在工作区内**，否则相对路径一律不信 —— 不加这条，`cd /etc && echo x > hosts` 会被当成本地只读放行（实测抓到的漏洞） |
| heredoc | 只把**带引号定界符**（`<<'EOF'`）的正文当数据剥掉：不带引号的定界符会做变量展开，正文里的 `$(...)` 会真的执行。且要求定界符出现在行尾，避免把 `echo "见 <<'EOF' 文档"` 误判成 heredoc（误判会删掉后面真正的命令） |
| 变量赋值 | `VAR=value` 与 `export VAR=value` 放行（只影响本条命令的环境） |

**deny 不剥离 heredoc 正文**：`bash <<'EOF' … rm -rf ~ … EOF` 是把正文当脚本执行，必须在 deny 层被拦住。
