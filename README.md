...
- **输出带理由**：要求模型输出 `ALLOW`，或 `DENY - <不超过 15 字的理由>`。理由会写进
  `decisions.jsonl` 的 `model_reason` 字段并进 `--stats` 的原因分布 —— 这样「模型为什么
  拒绝」有据可查（早期只存一个词，无从追查，遇到误判只能靠猜）。
- **`never_ask: true`（默认）：模型必须二选一，不允许把决定推给人。**
  模型原本的第三个选项「ASK（交人工）」等于让用户来点一下；设成 `never_ask` 后它只能
  ALLOW 或 DENY，**拒绝是硬阻断、不弹窗**。代价与恢复路径：被拒的命令会失败并把理由
  回给 agent，agent 会向你说明；**你确认后它重试即可放行**——因为重试时你那条确认
  已经进入 TRUSTED 上下文，会被重新评估（同一个命令在相同上下文下会命中拒绝缓存，
  不会反复重试）。
  想恢复旧行为（模型可以「交人工」）把 `never_ask` 设回 `false`。
